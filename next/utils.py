...
def get_supported_apps(apps_path="apps/"):
  ...
  """
    ...
    return [
        d
        for d in next(os.walk(os.path.dirname(apps_path)))[1]
        if d[0] not in {".", "_"}
    ]
...
def filenames_to_ids(filenames, targets):
    ...
    if len(not_in_targets) > 0:
        msg = (
            'Filenames specified in init.yaml "{}" in the not found the list of targets'
        )
        ...
        if type(a) in {str}:
            ...
            for line in lines:
                ...
                for line2 in pprint_arg:
                    print(
                        "{}{}{}".format(
                            color_ansi[color], line2, color_ansi["reset all"]
                        )
                    )
        else:
