...
class AppDashboard(object):
    ...
    """
        ...
        numerical_timestamps = [
            (
                utils.str2datetime(item["timestamp_query_generated"]) - start_date
            ).total_seconds()
            for item in queries
        ]
    ...
    """
        ...
        for algorithm in alg_list:
            ...
            for item in list_of_log_dict:
                ...
                x.append(k)
                y.append(
                    item.get("app_duration", 0.) + item.get("duration_enqueued", 0.)
                )
    ...
    def compute_duration_detailed_stacked_area_plot(
        self, app, butler, task, alg_label, detailedDB=False
    ):
        """
