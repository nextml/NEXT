...
def check_format(doc, rets=True):
    ...
    if rets:
        for x in doc:
            if "args" in doc[x]:
                errs += check_format_helper(
                    {"type": "dict", "values": doc[x]["args"]}, "args/" + x
                )
            if "rets" in doc[x]:
                errs += check_format_helper(
                    {"type": "dict", "values": doc[x]["rets"]}, "rets/" + x
                )
    else:
        for x in doc:
...
def check_format_helper(doc, name):
    ...
    if (
        not doc["type"]
        in DICT | LIST | TUPLE | ONEOF | NUM | STRING | BOOL | ANY | FILE
    ):
...
def verify_helper(name, input_element, reference_dict):
    ...
    if reference_dict["type"] in DICT:
        if not isinstance(input_element, (dict)):
            ...
            if len(l1) > 0:
                ans += [
                    {"name": name, "message": "extra keys in input: " + ",".join(l1)}
                ]
            else:
                ok = True
                for k in l2:
                    if "default" in reference_dict["values"][k]:
                        ...
                        not "optional" in reference_dict["values"][k]
                    ) or reference_dict["values"][k]["optional"] == False:
                        ans += [
                            {
                                "name": name + "/" + k,
        ...
        if not isinstance(input_element, (str, unicode)):
            ...
            "values" in reference_dict and not input_element in reference_dict["values"]
        ):
            ans += [
                {
                    "name": name,
                    "message": "argument must be one of the specified strings: "
            ...
            if k in input_element:
                ...
                if count > 1:
                    ans += [
                        {
                            "name": name + "/" + k,
                            "message": "More than one argument specified for 'oneof arg: "
                            + name,
                        }
                    ]
        if count == 0:
            if "default" in reference_dict:
                ...
                ans += [
                    {"name": name, "message": "no argument provided for 'oneof' arg"}
                ]
        ...
        ans += [
            {"name": name, "message": "invalid type: {}".format(reference_dict["type"])}
        ]
