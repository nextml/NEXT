...
def zipfile_to_dictionary(zip_file):
    ...
    filenames = [
        f
        for f in filenames
        if not any([ignore in f.lower() for ignore in ["ds_store", "icon", "__macosx"]])
    ]
...
def upload_target(
    filename, file_obj, bucket_name, aws_key, aws_secret_key, i=None, get_bucket=True
):
    if get_bucket:
    ...
    if filetype not in target_types:
        msg = (
            'Target not recognized (extension: "{}"). ' "Available extensions: {}"
        ).format(filetype, list(target_types.keys()))
...
def unpack(s, aws_key, aws_secret_key, bucket_name, n_jobs=None, get_bucket=True):
    base64_zip = io.BytesIO(s)
    zip_file = zipfile.ZipFile(base64_zip)
    ...
    try:
        targets = Parallel(n_jobs=n_jobs, backend="threading")(
            delayed(upload_target, check_pickle=False)(
                name, file, bucket_name, aws_key, aws_secret_key, i=i, get_bucket=True
            )
...
if __name__ == "__main__":
    ...
    with open("../../examples/strange_fruit_triplet/strangefruit30.zip", "rb") as f:
        s = f.read()
    print(s)
    targets = unpack(s, aws_key, aws_secret_access_key, bucket_name="scott_test")
    pprint(targets)
