...
class getQuery(Resource):
    def post(self):
        ...
        if "participant_uid" in args_data["args"].keys():
            args_data["args"]["participant_uid"] = (
                exp_uid + "_" + str(args_data["args"]["participant_uid"])
            )
        ...
        if not didSucceed:
            return attach_meta(
                {}, meta_error["QueryGenerationError"], backend_error=message
            )
        ...
        if render_widget:
            ...
            return (
                {"html": template.render(query=response_dict), "args": response_dict},
                200,
                {
