...
def args_summary(api):
    if api["type"] == "list":
        ...
        return "{{{}}}".format(
            ", ".join(
                [
                    "{}: {}".format(k, args_summary(api["values"][k]))
                    for k in api["values"]
                ]
            )
        )
...
def args_gen(api, depth):
    ...
    if api["type"] == "list":
        return "List, all of whose elements are as follows:  \n{indent}  * {elements}\n".format(
            indent=indent, elements=args_gen(api["values"], depth + 2)
        )
    elif api["type"] == "dict":
        return "Dictionary with the following keys:\n{keys}\n{indent}".format(
            ...
            keys="\n".join(
                [
                    "{indent}`{key}`:{value}  {desc}".format(
                        indent=indent + "* ",
                        key=k,
                        value=args_gen(api["values"][k], depth + 1),
                        desc=(
                            "\n    " + indent + api["values"][k]["description"]
                            if "description" in api["values"][k]
                            else ""
                        ),
                    )
                    for k in api["values"]
        ...
        if "values" in api and len(api["values"]) > 0:
            ...
            return "`string`{}".format(
                ", " + api["description"] if "description" in api else ""
            )
        ...
        return " | ".join(
            [args_gen(api["values"][k], depth + 1) for k in api["values"]]
        )
    else:
