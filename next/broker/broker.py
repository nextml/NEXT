...
class JobBroker:
    ...
    def applySyncByNamespace(self, app_id, exp_uid, alg_id, alg_label, task_name, args, targets=None, namespace=None, ignore_result=False,time_limit=0):
        """
        ...
        if next.constants.CELERY_ON:
            result = tasks.apply_sync_by_namespace.apply_async(args=[app_id,exp_uid,alg_id,alg_label, task_name, args, targets, namespace, job_uid, submit_timestamp, time_limit], queue=queue_name,soft_time_limit=soft_time_limit,time_limit=hard_time_limit)
            if ignore_result:
                return True
            else:
                return result.get(interval=.001) 
        else:
            result = tasks.apply_sync_by_namespace(app_id,exp_uid,alg_id,alg_label,task_name, args, targets, namespace, job_uid, submit_timestamp, time_limit)
            if ignore_result:
                return True
            else:
                return result
