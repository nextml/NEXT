...
class DatabaseAPI(object):
    ...
    def exists(self, bucket_id, doc_uid, key):
        ...
        doc = (
            self._bucket(bucket_id).find_one({"_id": doc_uid}, projection={key: True})
            or {}
        )
    ...
    def get_and_delete(self, bucket_id, doc_uid, key):
        doc = self._bucket(bucket_id).find_one_and_update(
            {"_id": doc_uid}, update={"$unset": {key: ""}}, projection={key: True}
        )
    ...
    def increment_many(self, bucket_id, doc_uid, key_value_dict):
        ...
        new_doc = self._bucket(bucket_id).find_one_and_update(
            {"_id": doc_uid},
            update={"$inc": values},
    ...
    def pop_list(self, bucket_id, doc_uid, key, end):
        ...
        val = (
            self._bucket(bucket_id)
            .find_and_modify({"_id": doc_uid}, {"$pop": {key: mongo_idx}})
            .get(key)
        )
    ...
    def set(self, bucket_id, doc_uid, key, value):
        self._bucket(bucket_id).update_one(
            {"_id": doc_uid}, {"$set": {key: to_db_fmt(value)}}, upsert=True
        )

    def set_many(self, bucket_id, doc_uid, key_value_dict):
        self._bucket(bucket_id).update_one(
            {"_id": doc_uid}, {"$set": to_db_fmt(key_value_dict)}
        )

    def delete(self, bucket_id, doc_uid, key):
        self._bucket(bucket_id).update_one({"_id": doc_uid}, {"$unset": {key: True}})
    ...
    def submit_job(
        ...
        if namespace is None:
            ...
            result = self.broker.applySyncByNamespace(
                app_id,
                exp_uid,
                alg_id,
                alg_label,
