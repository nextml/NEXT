...
from next.database_client.DatabaseAPI import (
    to_db_fmt,
    from_db_fmt,
    DatabaseAPI,
    DatabaseException,
...
def test_get(db):
    ...
    doc_uid = (
        db._bucket(B).insert_one({"a_key": 2, "another_key": [1.0, "f"]}).inserted_id
    )
...
def test_get_many(db):
    ...
    assert db.get_many(B, doc_uid, ["another_key"]) == {"another_key": [1.0, "f"]}
...
def test_increment_many(db):
    ...
    assert db.get_many(B, doc_uid, ["a", "b", "c"]) == {"a": 1, "b": 5, "c": -7}
...
def test_set_doc(db):
    ...
    assert db.get_doc(B, doc_uid) == {"_id": str(doc_uid), "a": 5, "b": "foo"}

    # add a new document with _id='asdf'
    db.set_doc(B, "asdf", {"a": 3, "b": "baz"})
    assert db.get_doc(B, "asdf") == {"_id": "asdf", "a": 3, "b": "baz"}
...
def test_get_docs_with_filter(db):
    ...
    db._bucket(B).insert_many([{"a": 3, "b": 2}, {"a": 5, "b": 2}, {"a": 1, "b": 3}])

    retrieved_docs = db.get_docs_with_filter(B, {"b": 2})
    # remove `_id`s for asserts
    retrieved_docs = [
        {k: v for k, v in r.items() if k != "_id"} for r in retrieved_docs
    ]
