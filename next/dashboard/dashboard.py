...
def experiment_list():
    ...
    return render_template(
        ...
        experiments=sorted(experiments, key=lambda e: e["start_date"], reverse=True),
    )
...
def system_monitor():
    ...
    """
    host_url = "http://{}:{}".format(
        constants.NEXT_BACKEND_GLOBAL_HOST, constants.NEXT_BACKEND_GLOBAL_PORT
    )

    rabbit_url = "http://{}:{}".format(constants.NEXT_BACKEND_GLOBAL_HOST, 15672)
    cadvisor_url = "http://{}:{}".format(constants.NEXT_BACKEND_GLOBAL_HOST, 8888)
    mongodb_url = "http://{}:{}".format(constants.NEXT_BACKEND_GLOBAL_HOST, 28017)
    return render_template(
        "system_monitor.html",
...
def retire_exp(exp_uid):
    retired = request.form.get(
        "retired", default=True, type=flask_restful.inputs.boolean
    )
