...
def run_all(assert_200):
  ...
  app_id = 'ConstrainedRegression'
  num_arms = 50

  # Exponential
  discount_factor = 3
  true_means = numpy.exp(numpy.multiply(-discount_factor, numpy.arange(num_arms)/float(num_arms)))

  # Hyperbolic
  # discount_factor = 5
  # true_means = numpy.divide(1, (1.0 + discount_factor.*numpy.arange(num_arms)/float(num_arms)))
...
def simulate_one_client( input_args ):
  ...
  for t in range(total_pulls):
    ...
    target_reward = float(numpy.random.rand()<true_means[target_index]) # in {1,2,3}
