...
class MyAppDashboard(AppDashboard):
    ...
    def test_error_multiline_plot(self, app, butler):
        ...
        test_queries = butler.db.get_docs_with_filter(
            app.app_id + ":queries",
            {"exp_uid": app.exp_uid, "alg_label": test_alg_label},
        )

        test_S = [
            (query["target_index"], query["target_label"])
        ...
        for algorithm in alg_list:
            ...
            list_of_log_dict = self.ell.get_logs_with_filter(
                ...
                {"exp_uid": app.exp_uid, "alg_label": alg_label},
            )
            ...
            for item in list_of_log_dict:
                ...
                for q in test_S:
                    estimated_label = numpy.sign(
                        numpy.dot(
                            numpy.array(target_features[q[0]]), numpy.array(weights)
                        )
                    )
