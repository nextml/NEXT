...
class MyAlg:
    ...
    def getQuery(self, butler):
        ...
        while time.time() - t_start < t_max:
            ...
            for i in range(n):
                p += (
                    utilsCrowdKernel.getCrowdKernelTripletProbability(X[b], X[c], X[i])
                    * tau[a, i]
                )

            taub = list(tau[a])
            for i in range(n):
                taub[i] = taub[i] * utilsCrowdKernel.getCrowdKernelTripletProbability(
                    X[b], X[c], X[i]
                )
            taub = taub / sum(taub)
            ...
            tauc = list(tau[a])
            for i in range(n):
                tauc[i] = tauc[i] * utilsCrowdKernel.getCrowdKernelTripletProbability(
                    X[c], X[b], X[i]
                )
            tauc = tauc / sum(tauc)

            entropy = -p * utilsCrowdKernel.getEntropy(taub) - (
                1 - p
            ) * utilsCrowdKernel.getEntropy(tauc)
