...
def getLoss(X, S):
    ...
    for q in S:
        ...
        log_loss = log_loss - log(
            getCrowdKernelTripletProbability(X[q[0]], X[q[1]], X[q[2]], mu=.01)
        )
...
def getCrowdKernelTauDistribution(X, S, mu=.01):
    ...
    for q in S:
        ...
        for i in range(n):
            tau[a, i] = tau[a, i] + log(
                getCrowdKernelTripletProbability(X[q[0]], X[q[1]], X[i], mu=mu)
            )
...
def getGradient(X, S, mu):
    ...
    for q in S:
        i, j, k = q
        num = max(
            mu + norm(X[k] - X[j]) * norm(X[k] - X[j]), realmin
        )  # max(mu + D[k,j] , realmin)
        den = max(
            2 * mu
            + norm(X[k] - X[i]) * norm(X[k] - X[i])
            + norm(X[k] - X[j]) * norm(X[k] - X[j]),
            realmin,
...
def computeEmbeddingWithGD(
    X, S, mu, max_iters=0, max_norm=0, epsilon=0.01, c1=0.0001, rho=.7, verbose=False
):
    """
    ...
    while t < max_iters:
        ...
        if verbose:
            print(
                "iter=%d,   emp_loss=%f,   hinge_loss=%f,   log_loss=%f,   rel_avg_grad=%f,   rel_max_grad=%f,   a=%f,   i_t=%d"
                % (
                    t,
                    emp_loss_k,
                    hinge_loss_k,
