...
class MyApp:
    ...
    def processAnswer(self, butler, alg, args):
        ...
        q = (
            [left_id, right_id, center_id]
            if target_winner == left_id
            else [right_id, left_id, center_id]
        )
    ...
    def format_responses(self, responses):
        ...
        for response in responses:
            if "target_winner" not in response:
                continue
            targets = {
                "target_" + target["label"]: target["primary_description"]
                for target in response["target_indices"]
            }
            ids = {
                target["label"] + "_id": target["target_id"]
                for target in response["target_indices"]
            }
            winner = {
                t["target_id"]
                == response["target_winner"]: (t["primary_description"], t["target_id"])
