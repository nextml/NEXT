...
def test_validation_params():
    params = [{"num_tries": 5}, {"query_list": [[0, 1], [1, 2], [3, 4]]}]
    for param in params:
        print(param)
...
def test_api(
    ...
    for i in range(num_clients):
        ...
        pool_args.append(
            (exp_uid, participant_uid, total_pulls_per_client, true_means, assert_200)
        )
...
def simulate_one_client(input_args):
    ...
    for t in range(total_pulls):
        ...
        getQuery_args_dict = {
            "args": {"participant_uid": participant_uid, "widget": widget},
            "exp_uid": exp_uid,
        }
